...
view: uk_path_analyzer {
  ...
  derived_table: {
    sql: WITH
        ...
        {% if uk_path_analyzer.first_event_selector._in_query %}
        -- Find paths that contain the first event, and locate the first occurrence of that event
          , first_event_selector AS (
            ...
            FROM ${uk_exploded_paths.SQL_TABLE_NAME}
        ...
        {% if uk_path_analyzer.last_event_selector._in_query %}
        ...
        , last_event_selector AS (
            ...
            FROM ${uk_exploded_paths.SQL_TABLE_NAME} ep
            {% if uk_path_analyzer.first_event_selector._in_query %}
        ...
        , sub_paths AS (
            ...
            {% endif %}
            WHERE ep.country = 'UK'  -- Filter for UK data
            GROUP BY
                pc.count
              , pc.path
          )
      ...
      -- Sum everything up to find counts within the sub-path
        ...
        FROM {% if uk_path_analyzer.last_event_selector._in_query or uk_path_analyzer.first_event_selector._in_query %}
                sub_path_summary
             {% else %}
                ${path_counts.SQL_TABLE_NAME}
             {% endif %}
        WHERE {% if uk_path_analyzer.last_event_selector._in_query or uk_path_analyzer.first_event_selector._in_query %}
                TRUE
              {% else %}
                country = 'UK'
              {% endif %}
