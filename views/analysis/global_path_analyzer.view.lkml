...
view: global_path_analyzer {
  ...
  derived_table: {
    sql: WITH
        ...
        {% if global_path_analyzer.first_event_selector._in_query %}
        -- Find paths that contain the first event, and locate the first occurrence of that event
          , first_event_selector AS (
            ...
            FROM ${global_exploded_paths.SQL_TABLE_NAME}
        ...
        {% if global_path_analyzer.last_event_selector._in_query %}
        ...
        , last_event_selector AS (
            ...
            FROM ${global_exploded_paths.SQL_TABLE_NAME} ep
            {% if global_path_analyzer.first_event_selector._in_query %}
        ...
        , sub_paths AS (
            SELECT
              ...
              , ep.country  -- Include country field
            FROM ${global_exploded_paths.SQL_TABLE_NAME} ep
            ...
            GROUP BY
                pc.count
              , pc.path
              , ep.country  -- Include in GROUP BY
          )
      ...
      -- Sum everything up to find counts within the sub-path
        , sub_path_summary as (
            ...
            GROUP BY
                path
              , country  -- Include in GROUP BY
          )

        SELECT
          *
        -- If there aren't fitlers selected, go straight to the pre-built table
        FROM {% if global_path_analyzer.last_event_selector._in_query or global_path_analyzer.first_event_selector._in_query %}
                sub_path_summary
             {% else %}
